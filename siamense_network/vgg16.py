...
class Vgg16:
    ...
    def build_as_simense(self, rgb, reuse=False):

        """
        load variable from npy to build the VGG

        :param rgb: rgb image [batch, height, width, 3] values scaled [0, 1]
        :param reuse: whether to reuse the net
        """
        ...
        print(("build vgg model with pretrained weights finished: %ds" % (time.time() - start_time)))
        return self.relu6
    ...
    def fc_layer(self, bottom, name, reuse=False):
        with tf.variable_scope(name, reuse=reuse):
